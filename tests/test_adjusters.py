...
class TestOpenAICompanyResearchAdjuster:
    ...
    def test_adjust_calls_openai_client(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    def test_adjust_build_system_prompt_returns_none(
        self, mock_format_prompt, mock_research, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_openai_returns_null(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_schema_validation_exception(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
        ...
        """adjust should return adjusted CV without schema validation."""
        # Setup mocks
        mock_research.return_value = {"company": "Test Corp"}
        mock_format_prompt.return_value = "System prompt for Test Corp"
        ...
        # Should return adjusted CV
        assert result_data == adjusted_data
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_validation_fails(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_non_dict_result(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_research_returns_falsy(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_with_technology_signals(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_with_invalid_confidence_value(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_openai_api_exception(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_json_decode_exception(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
    ...
    @patch("cvextract.adjusters.openai_company_research_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_company_research_adjuster.OpenAI")
    def test_adjust_validation_result_has_errors_count(
        self,
        mock_openai,
        mock_format_prompt,
        mock_research,
        ...
        """adjust should return adjusted data without schema validation."""
        # Setup mocks
        mock_research.return_value = {"name": "Test Corp"}
        mock_format_prompt.return_value = "System prompt"
...
class TestOpenAIJobSpecificAdjuster:
    ...
    def test_validate_params_requires_job_url_or_description(self):
        ...
        adjuster = OpenAIJobSpecificAdjuster()
        with pytest.raises(
            ValueError, match="requires either 'job-url' or 'job-description'"
        ):
            adjuster.validate_params()
    ...
    def test_validate_params_rejects_empty_job_url_and_description(self):
        ...
        adjuster = OpenAIJobSpecificAdjuster()
        with pytest.raises(
            ValueError, match="requires either non-empty 'job-url' or 'job-description'"
        ):
    ...
    def test_adjust_openai_becomes_none_after_initial_check(
        ...
        try:
            ...
            with patch(
                "cvextract.adjusters.openai_job_specific_adjuster.format_prompt"
            ) as mock_format:
    ...
    def test_adjust_second_openai_check_lines_160_161(
        self, monkeypatch, tmp_path: Path
    ):
    ...
    def test_adjust_with_openai_not_installed(self, monkeypatch, tmp_path: Path):
        ...
        monkeypatch.setattr(
            "cvextract.adjusters.openai_job_specific_adjuster.OpenAI", None
        )
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_api_call_success(
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_empty_completion(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_invalid_json_response(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_empty_choices_list(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_retry_on_transient_error(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
        ...
        mock_completion_success.choices = [
            MagicMock(message=MagicMock(content=json.dumps(adjusted_cv)))
        ]
        ...
        result = adjuster.adjust(work, job_description="Test job")
        assert (
            read_output(result) == adjusted_cv
        ), f"Expected adjusted CV, got {read_output(result)}"
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_retry_exhausts_max_attempts(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_non_transient_error_no_retry(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_json_parsed_to_none(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI", None)
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_openai_none_before_client_creation(
        self, mock_format, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_with_retry_backoff_sleep(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_completion_choices_false(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_with_both_job_url_and_description(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
        ...
        mock_format.assert_called_once_with(
            "adjuster_promp_for_specific_job", job_description="Direct description"
        )

    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_rate_limit_with_429_in_error_message(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
        ...
        assert (
            read_output(result) == adjusted_cv
        ), f"Expected adjusted CV with 'Adjusted' name, got {read_output(result)}"
        # Should have retried after seeing 429
        assert mock_client.chat.completions.create.call_count == 2

    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_final_retry_attempt_rate_limit(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_schema_validation_fails(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
        """adjust should return adjusted CV without schema validation."""
        mock_format.return_value = "System prompt"
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_returns_adjusted_cv_without_schema_validation(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
        """adjust should return adjusted CV without schema validation."""
        mock_format.return_value = "System prompt"
    ...
    def test_adjust_schema_validation_exception(
        ...
        # Should return adjusted CV without schema validation
        work = make_work(tmp_path, cv_data)
        result = adjuster.adjust(work, job_description="Test job")
        assert read_output(result) == adjusted_cv
    ...
    def test_parse_multiple_adjust_flags(self):
        ...
        config = gather_user_requirements(
            [
                ...
                "job-url=https://careers.example.com/job/123",
                "--target",
                "/output",
            ]
        )
        ...
        assert (
            config.adjust.adjusters[1].params.get("job-url")
...
class TestOpenAICompanyResearchHelpers:
    ...
    def test_fetch_customer_page_success(self):
        ...
        with patch(
            "cvextract.adjusters.openai_company_research_adjuster.requests.get",
    ...
    def test_validate_research_data_accepts_valid_payload(self):
        """_validate_research_data() returns True for valid research payload."""
        from cvextract.adjusters.openai_company_research_adjuster import (
            _validate_research_data,
        )

        result = _validate_research_data({"name": "Test Corp", "domains": ["tech"]})
        assert result is True

    def test_validate_research_data_rejects_missing_required_fields(self):
        """_validate_research_data() returns False when required fields are missing."""
        from cvextract.adjusters.openai_company_research_adjuster import (
            _validate_research_data,
        )
...
class TestCompanyResearchRetryHelpers:
    ...
    def test_sleep_with_backoff_uses_jitter_when_not_deterministic(self):
        ...
        with patch("cvextract.openai_utils.random.random", return_value=0.5):
            retryer._sleep_with_backoff(0, is_write=False, exc=exc)
...
class TestCompanyResearchSchemaLoading:
    ...
    def test_load_research_schema_open_failure(self, tmp_path):
        ...
        try:
            ...
            adj_module._RESEARCH_SCHEMA = None
            with patch(
                "cvextract.adjusters.openai_company_research_adjuster.open",
...
class TestResearchCompanyProfileFailures:
    ...
    def test_research_company_profile_skips_when_prompt_missing(self):
        ...
        with patch(
            ...
            with patch(
                "cvextract.adjusters.openai_company_research_adjuster.format_prompt",
                return_value=None,
            ):
                result = _research_company_profile(
                    "https://example.com",
    ...
    def test_research_company_profile_empty_completion(self):
        ...
        with patch(
            "cvextract.adjusters.openai_company_research_adjuster.OpenAI",
    ...
    def test_research_company_profile_validation_failure(self):
        ...
        mock_completion = MagicMock()
        mock_completion.choices = [
            MagicMock(message=MagicMock(content='{"name": "Bad Co"}'))
        ]
        mock_client = MagicMock()
        mock_client.chat.completions.create.return_value = mock_completion
...
class TestOpenAIJobSpecificAdjusterEdgeCases:
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    @patch("cvextract.adjusters.openai_job_specific_adjuster._fetch_job_description")
    def test_adjust_uses_both_prompt_template_names(
        self, mock_fetch, mock_format, mock_openai_class, tmp_path: Path
    ):
        ...
        mock_completion.choices = [
            MagicMock(
                message=MagicMock(
                    content='{"identity": {"name": "", "title": "", "full_name": "", "first_name": "", "last_name": ""}, "sidebar": {}, "overview": "", "experiences": []}'
                )
            )
        ]
        mock_client = MagicMock()
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_validation_error_with_error_count(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
        """adjust should return adjusted CV without schema validation."""
        mock_format.return_value = "System prompt"
    ...
    @patch("cvextract.adjusters.openai_job_specific_adjuster.OpenAI")
    @patch("cvextract.adjusters.openai_job_specific_adjuster.format_prompt")
    def test_adjust_completion_with_multiple_choices(
        self, mock_format, mock_openai_class, tmp_path: Path
    ):
