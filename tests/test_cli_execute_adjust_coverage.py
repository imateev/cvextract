...
class TestCliExecuteAdjustCoverage:
    ...
    def test_execute_handles_source_from_render_data(self, tmp_path):
        ...
        config = UserConfig(
            ...
            adjust=AdjustStage(
                data=None, adjusters=[AdjusterConfig(name="test-adjuster", params={})]
            ),
        )
    ...
    def test_execute_returns_base_work_when_output_is_not_file(self, tmp_path):
        """execute should return base work when output is not a file."""
    ...
    def test_execute_applies_delay_between_adjusters(self, tmp_path):
        ...
        with patch("cvextract.cli_execute_adjust.get_adjuster") as mock_get, patch(
            "cvextract.cli_execute_adjust.time.sleep"
        ) as mock_sleep:
            mock_adjuster = MagicMock()
            mock_adjuster.validate_params = MagicMock()
            mock_adjuster.adjust = MagicMock(return_value=work)
    ...
    def test_execute_handles_validation_error(self, tmp_path):
        ...
        config = UserConfig(
            ...
            adjust=AdjustStage(
                data=json_file,
                adjusters=[
                    AdjusterConfig(name="test-adjuster", params={"invalid": "param"})
                ],
            ),
        )
