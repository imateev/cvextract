...
def full_template(tmp_path):
    ...
    doc.add_paragraph(
        "Languages: {% for lang in sidebar.languages %}{{ lang }}{% if not loop.last %}, {% endif %}{% endfor %}"
    )
    doc.add_paragraph(
        "Tools: {% for tool in sidebar.tools %}{{ tool }}{% if not loop.last %}, {% endif %}{% endfor %}"
    )
...
class TestDocxCVRendererBasicFunctionality:
    ...
    def test_render_creates_output_file(
        self, minimal_template, tmp_path, make_render_work
    ):
...
class TestDocxCVRendererDataRendering:
    ...
    def test_renders_complete_cv_data(
        self, full_template, sample_cv_data, tmp_path, make_render_work
    ):
    ...
    def test_renders_with_empty_optional_fields(
        self, minimal_template, tmp_path, make_render_work
    ):
...
class TestDocxCVRendererSpecialCharacters:
    ...
    def test_handles_special_xml_characters(
        self, minimal_template, tmp_path, make_render_work
    ):
    ...
    def test_sanitizes_non_breaking_spaces(
        self, minimal_template, tmp_path, make_render_work
    ):
...
class TestDocxCVRendererOutputDirectory:
    ...
    def test_works_with_existing_output_directory(
        self, minimal_template, tmp_path, make_render_work
    ):
...
class TestDocxCVRendererComplexData:
    ...
    def test_renders_multiple_experiences(
        self, full_template, tmp_path, make_render_work
    ):
    ...
    def test_renders_with_all_sidebar_fields(
        self, full_template, sample_cv_data, tmp_path, make_render_work
    ):
...
class TestDocxCVRendererEdgeCases:
    ...
    def test_renders_with_none_environment(
        self, full_template, tmp_path, make_render_work
    ):
