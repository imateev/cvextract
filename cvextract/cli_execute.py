...
def execute_pipeline(config: UserConfig) -> int:
    ...
    if config.mode:
        return _execute_legacy_pipeline(config)
    else:
        return _execute_stage_pipeline(config)


def _execute_legacy_pipeline(config: UserConfig) -> int:
    """Execute pipeline using legacy mode-based configuration."""
    # Collect inputs
    try:
        inputs = _collect_inputs(config.source, config.mode, config.has_extract, config.template)
...
def _execute_stage_pipeline(config: UserConfig) -> int:
    """Execute pipeline using new stage-based configuration."""
    ...
    try:
        ...
        inputs = _collect_inputs(source, None, is_extraction, template_path)
    except Exception as e:
        LOG.error(str(e))
        if config.debug:
            LOG.error(traceback.format_exc())
