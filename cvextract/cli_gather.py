...
""",
    ...
    parser.add_argument(
        "--list",
        choices=["adjusters", "renderers", "extractors"],
        help="List available components: adjusters, renderers, or extractors",
    )
    parser.add_argument(
        "--target", help="Target output directory (required unless using --list)"
    )
    parser.add_argument(
        "--verbosity",
    ...
    using_stages = any(
        [
            ...
            args.adjust is not None,
            args.render is not None,
    ...
    if not using_stages:
        raise ValueError(
            "Must specify at least one stage flag (--extract, --adjust, --render, or --parallel)"
        )
    ...
    if args.parallel is not None:
        ...
        if "n" in params:
            try:
                ...
                raise ValueError(
                    f"--parallel parameter 'n' must be a valid integer: {e}"
                )
    ...
    if args.adjust is not None:
        ...
        for adjust_params_list in args.adjust:
            ...
            if not adjuster_name:
                raise ValueError(
                    "--adjust requires 'name' parameter to specify the adjuster"
                )
